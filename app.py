...
if uploaded_file is not None:
    # Ajoutez un bouton pour démarrer la génération du tableau
    if st.button('Générer le tableau'):
        ...
        with pdfplumber.open(uploaded_file) as pdf:
            ...
            if tables:
                ...
                st.session_state['df'] = df  # Enregistrer le DataFrame dans l'état de session

# Ajoutez un bouton pour démarrer la génération des documents Word
if 'df' in st.session_state:
    if st.button('Générer les documents Word'):
        ...
        # Utiliser le DataFrame enregistré dans l'état de session
